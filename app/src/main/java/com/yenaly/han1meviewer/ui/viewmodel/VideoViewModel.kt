...
class VideoViewModel(application: Application) : YenalyViewModel(application) {
    ...
    viewModelScope.launch {
      ...
      NetworkRepo.getVideoDetail(code)
        ...
        .collectLatest { video ->
          ...
          video.relatedHanimes.forEach { relatedVideo -> textsToTranslate.add(relatedVideo.title) }
  ...
  fun refreshTranslations(video: HanimeVideo) {
    viewModelScope.launch {
      ...
      video.playlist?.video?.forEach { playlistVideo -> textsToTranslate.add(playlistVideo.title) }
