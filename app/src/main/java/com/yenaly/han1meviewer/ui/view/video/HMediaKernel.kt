  ...
  override fun prepare() {
    ...
    val dataSourceFactory =
      DefaultDataSource.Factory(
        context,
        DefaultHttpDataSource.Factory().setDefaultRequestProperties(jzvd.jzDataSource.headerMap),
      )

    val currUrl = jzvd.jzDataSource.currentUrl.toString()
    val videoSource =
      if (currUrl.contains(".m3u8")) {
        HlsMediaSource.Factory(dataSourceFactory).createMediaSource(MediaItem.fromUri(currUrl))
      } else {
...
class MpvMediaKernel(jzvd: Jzvd) : JZMediaInterface(jzvd), HMediaKernel, IMedia {
  ...
  fun setSuperResolution(index: Int) {
    if (index != 0) {
      val cmd =
        arrayOf("change-list", "glsl-shaders", "set", AnimeShaders.getShader(jzvd.context, index))
      MPVLib.command(cmd)
    } else {
      clearSuperResolution()
    }
