...
class HKeyframesRvAdapter : BaseDifferAdapter<HKeyframeEntity, QuickViewHolder>(COMPARATOR) {
  ...
  companion object {
    val COMPARATOR =
      object : DiffUtil.ItemCallback<HKeyframeEntity>() {
        ...
        override fun areContentsTheSame(oldItem: HKeyframeEntity, newItem: HKeyframeEntity) =
          oldItem == newItem
      }
  ...
  override fun onBindViewHolder(holder: QuickViewHolder, position: Int, item: HKeyframeEntity?) {
    ...
    holder.getView<RecyclerView>(R.id.rv_h_keyframe).apply {
      ...
      adapter = HKeyframeRvAdapter(item.videoCode, item).apply { isLocal = item.author == null }
    }
  }
  ...
  private fun delete(item: HKeyframeEntity) {
    ...
    if (activity is SettingsActivity) {
      activity.showAlertDialog {
        ...
        setPositiveButton(R.string.confirm) { _, _ -> activity.viewModel.deleteHKeyframes(item) }
        setNegativeButton(R.string.cancel, null)
      }
    }
  }
