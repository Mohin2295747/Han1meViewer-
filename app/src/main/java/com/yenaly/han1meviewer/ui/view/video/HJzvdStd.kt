...
class HJzvdStd @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
  ...
  override fun touchActionMove(x: Float, y: Float) {
    ...
    if (screen != SCREEN_TINY && !isSpeedGestureDetected) {
      ...
      if (
        mDownX > JZUtils.getScreenWidth(context) || mDownY < JZUtils.getStatusBarHeight(context)
      ) {
        return
      }
  ...
  private fun changeUiToPreparingPlayingShow() {
    when (screen) {
      SCREEN_NORMAL,
      SCREEN_FULLSCREEN -> {
        setAllControlsVisiblity(VISIBLE, VISIBLE, INVISIBLE, VISIBLE, INVISIBLE, VISIBLE, INVISIBLE)
      }
    }
  }
  ...
  fun clickSpeed() {
    ...
    val popup =
      PopupWindow(inflater, JZUtils.dip2px(jzvdContext, 240f), LayoutParams.MATCH_PARENT, true)
        .apply {
