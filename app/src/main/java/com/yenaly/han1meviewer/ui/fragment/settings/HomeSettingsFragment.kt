...
class HomeSettingsFragment :
  ...
  override fun onPreferencesCreated(savedInstanceState: Bundle?) {
    videoLanguage.apply {
      ...
      // 從 xml 轉移至此
      entries =
        arrayOf(getString(R.string.traditional_chinese), getString(R.string.simplified_chinese))
      ...
      setOnPreferenceChangeListener { _, newValue ->
        if (newValue != Preferences.videoLanguage) {
          requireContext().showAlertDialog {
            ...
            setMessage(
              getString(R.string.restart_or_not_working, getString(R.string.video_language))
            )
          ...
          when (state) {
            ...
            is WebsiteState.Success -> {
              if (state.info == null) {
                ...
                update.summary = getString(R.string.check_update_success, state.info.version)
                update.setOnPreferenceClickListener {
                  viewLifecycleOwner.lifecycleScope.launch {
  ...
  private fun showUpdateFailedDialog(context: Context) {
    context.showAlertDialog {
      ...
      setPositiveButton(R.string.take_me_to_download) { _, _ -> browse(HA1_GITHUB_RELEASES_URL) }
      setNegativeButton(R.string.cancel, null)
    }
  }
