    ...
    return QuickViewHolder(R.layout.item_h_subscription, parent).apply {
      ...
      itemView.setOnLongClickListener {
        if (context is SearchActivity) {
          ...
          if (btnDelete.isGone) {
            submitList(items.map { it.copy(isDeleteVisible = true, isCheckBoxVisible = false) })
          } else {
            submitList(
              items.map {
                it.copy(
      ...
      getView<View>(R.id.btn_delete).setOnClickListener {
        ...
        context.showAlertDialog {
          ...
          setPositiveButton(R.string.confirm) { _, _ ->
            if (context is SearchActivity) {
              context.myListViewModel.subscription.deleteSubscription(item.artistId, position)
            }
          }
