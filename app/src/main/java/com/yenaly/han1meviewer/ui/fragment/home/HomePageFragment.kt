...
class HomePageFragment :
  ...
  override fun initData(savedInstanceState: Bundle?) {
    ...
    if (binding.rv.layoutManager is GridLayoutManager) {
      ...
      binding.rv.addItemDecoration(
        GridSpacingDecoration(spanCount = 2, spacing = 16.dpToPx(), start = 1, includeEdge = false)
      )
    }
      ...
      binding.cover.load(banner.picUrl) {
        ...
        target(
          ...
          onSuccess = {
            ...
            it.toBitmapOrNull()?.let(Palette::Builder)?.generate { p -> p?.let(::handlePalette) }
          },
        )
      }
  ...
  private fun handlePalette(p: Palette) {
    bindingOrNull?.let { binding ->
      ...
      val buttonBgColor = p.darkVibrantSwatch?.rgb ?: p.darkMutedSwatch?.rgb ?: Color.TRANSPARENT
