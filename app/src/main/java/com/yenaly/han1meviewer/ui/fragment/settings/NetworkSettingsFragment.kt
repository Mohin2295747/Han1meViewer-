...
class NetworkSettingsFragment :
  ...
  override fun onPreferencesCreated(savedInstanceState: Bundle?) {
    proxy.apply {
      summary =
        generateProxySummary(Preferences.proxyType, Preferences.proxyIp, Preferences.proxyPort)
  ...
  inner class ProxyDialog(proxyPref: Preference, @LayoutRes layoutRes: Int) {
    ...
    private fun initView() {
      ...
      cgTypes.setOnCheckedStateChangeListener { _, checkedIds -> enableView(checkedIds.first()) }
    }
