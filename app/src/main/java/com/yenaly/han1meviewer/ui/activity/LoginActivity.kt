    ...
    private fun handleLogin() {
      lifecycleScope.launch {
        NetworkRepo.login(username, password).collect { state ->
          dialog.getButton(AlertDialog.BUTTON_POSITIVE).isEnabled = state !is WebsiteState.Loading
          dialog.getButton(AlertDialog.BUTTON_NEGATIVE).isEnabled = state !is WebsiteState.Loading
          when (state) {
