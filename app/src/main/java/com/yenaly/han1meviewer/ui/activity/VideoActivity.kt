...
class VideoActivity :
        ...
        viewModel.hanimeVideoStateFlow.collect { state ->
          when (state) {
            ...
            is VideoLoadingState.Success -> {
              ...
              binding.videoPlayer.posterImageView.load(state.info.coverUrl) { crossfade(true) }
              // 將觀看記錄保存數據庫
              if (!fromDownload) {
                val entity =
                  WatchHistoryEntity(
  ...
  private fun initViewPager() {
    ...
    binding.videoTl.attach(binding.videoVp) { tab, position -> tab.setText(tabNameArray[position]) }
  }
  ...
  private fun registerReceiver() {
    ...
    activity?.let {
      ContextCompat.registerReceiver(it, playbackReceiver, filter, ContextCompat.RECEIVER_EXPORTED)
    }
  }
