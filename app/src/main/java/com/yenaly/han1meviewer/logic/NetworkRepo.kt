...
object NetworkRepo {
    ...
    pageIOFlow(
      request = {
        when (typeOrCode) {
          ...
          is MyListType -> HanimeNetwork.myListService.getMyListItems(page, typeOrCode.value)
  ...
  fun getSubscriptions(page: Int) =
    pageIOFlow(
      request = { HanimeNetwork.myListService.getMyListItems(page, MyListType.SUBSCRIPTION.value) },
  ...
  fun deleteMyListItems(typeOrCode: Any, videoCode: String, position: Int, token: String?) =
    websiteIOFlow(
      request = {
        when (typeOrCode) {
          is String ->
            HanimeNetwork.myListService.deleteMyListItems(typeOrCode, videoCode, csrfToken = token)
  ...
  fun getPlaylists() =
    websiteIOFlow(request = HanimeNetwork.myListService::getPlaylists, action = Parser::playlists)
  ...
  fun postComment(
    ...
    websiteIOFlow(
      request = {
        HanimeNetwork.commentService.postComment(csrfToken, currentUserId, type, targetUserId, text)
      }
    ) {
  ...
  fun postCommentReply(csrfToken: String?, replyCommentId: String, text: String) =
    websiteIOFlow(
      request = { HanimeNetwork.commentService.postCommentReply(csrfToken, replyCommentId, text) }
    ) {
  ...
  fun login(email: String, password: String) =
    flow {
        ...
        if (req.isSuccessful) {
          ...
          // 雙重保險
          emit(
            WebsiteState.Error(IllegalStateException(getString(R.string.account_or_password_wrong)))
          )
        }
      }
