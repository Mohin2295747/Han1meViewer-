...
fun TextView.setResizableText(
  ...
  val width = width
  if (width <= 0) {
    post { setResizableText(fullText, maxLines, viewMore, applyExtraHighlights) }
    return
  }
  ...
  if (textLayout.lineCount <= maxLines || adjustedText.isEmpty()) {
    // No need to add 'read more' / 'read less' since the text fits just as well (less than max
    // lines #).
  ...
  val lastHasNewLine =
    adjustedText
      .substring(textLayout.getLineStart(maxLines - 1), textLayout.getLineEnd(maxLines - 1))
      .contains("\n")
  val linedText =
    if (lastHasNewLine) {
      val charactersPerLine =
        textLayout.getLineEnd(0) /
          (textLayout.getLineWidth(0) / textLayout.ellipsizedWidth.toFloat())
  ...
  // Check if we perhaps need to even add characters? Happens very rarely, but can be possible if
  // there was a long word just wrapped
  ...
  val modifier: Int
  if (
    shortenedStringWithSuffixLayout.getLineEnd(maxLines - 1) >= shortenedStringWithSuffix.length
  ) {
    modifier = 1
...
private fun TextView.addClickablePartTextResizable(
  ...
  if (clickableText != null) {
    ...
    // val startIndexOffset = if (viewMore) 4 else 0 // Do not highlight the 3 dots and the
    // space
