...
object HanimeDownloadManager {
  ...
  internal fun notify(entity: HanimeDownloadEntity) =
    notify(HanimeDownloadWorker.Args.fromEntity(entity))
  ...
  private fun notify(args: HanimeDownloadWorker.Args) {
    when (args) {
      in downloadingQueue -> {
        ...
        waitingQueue.poll()?.let { waitingTask -> addTask(waitingTask, redownload = false) }
      }
  ...
  /** 添加下载任务，如果任务正在下载中，则不会重复添加； 如果下载队列已满，则会加入等待队列。 */
  ...
  /** 恢复下载任务，如果任务已经在下载队列中，则不会重复添加； 如果下载队列已满，则会将一个正在下载的任务放到等待队列， 然后将 [entity] 加入下载队列。 */
  fun resumeTask(entity: HanimeDownloadEntity) =
    resumeTask(HanimeDownloadWorker.Args.fromEntity(entity))

  /** 恢复下载任务，如果任务已经在下载队列中，则不会重复添加； 如果下载队列已满，则会将一个正在下载的任务放到等待队列， 然后将 [args] 加入下载队列。 */
  ...
  /** 停止下载任务，如果任务正在下载中，则会立即停止； 如果任务在等待队列中，则会从等待队列中移除。 */
  ...
  /** 停止下载任务，如果任务正在下载中，则会立即停止； 如果任务在等待队列中，则会从等待队列中移除。 */
  fun stopTask(entity: HanimeDownloadEntity): Operation =
    stopTask(HanimeDownloadWorker.Args.fromEntity(entity))

  /** 删除下载任务，如果任务正在下载中，则会立即停止并删除； 如果任务在等待队列中，则会从等待队列中移除。 */
  ...
  private fun startWork(
    ...
    HanimeDownloadWorker.build {
      ...
      .apply {
        workManager.beginUniqueWork(args.videoCode, ExistingWorkPolicy.REPLACE, this).enqueue()
      }
  }
  ...
  private class DownloadQueue private constructor(private val tag: String) {
    ...
    companion object {
      @JvmStatic fun withTag(tag: String) = DownloadQueue("DownloadQueue-$tag")
    }
