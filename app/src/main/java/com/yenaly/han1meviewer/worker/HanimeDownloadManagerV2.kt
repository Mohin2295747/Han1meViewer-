...
/** 优化后的下载管理器，利用 Channel 和 Semaphore 限制并发下载数， 同时通过监听 WorkManager 的任务状态实现“等待任务完成后释放许可”的逻辑。 */
object HanimeDownloadManagerV2 {
        ...
        when (msg) {
          is DownloadMsg.Add -> {
            if (msg.args.videoCode in activeDownloads) {
              ...
              if (
                activeDownloads.size < maxConcurrentDownloadCount &&
          ...
          is DownloadMsg.Resume -> {
            if (msg.args.videoCode in activeDownloads) {
              ...
              while (
                activeDownloads.size >= maxConcurrentDownloadCount && activeDownloads.isNotEmpty()
              ) {
          ...
          is DownloadMsg.Stop -> {
            if (activeDownloads.remove(msg.args.videoCode) != null) {
              ...
              //                            waitingQueue.removeIf { it.videoCode ==
              // msg.args.videoCode }
            }
          }
  ...
  suspend fun init() {
    Log.d(TAG, "init")
    val allDownloading = DownloadDatabase.instance.hanimeDownloadDao.loadAllDownloadingHanimeOnce()
      ...
      .onFailure { t -> // 上述方法可能无法取消任务
        ...
        val deleteRequest =
          OneTimeWorkRequestBuilder<HanimeDownloadWorker>()
            ...
            .build()
        workManager
          .beginUniqueWork(args.videoCode, ExistingWorkPolicy.REPLACE, deleteRequest)
          .enqueue()
          .await()
  ...
  private suspend fun awaitWorkCompletion(videoCode: String) = suspendCancellableCoroutine { cont ->
