 ...
 */
class HUpdateWorker(private val context: Context, workerParams: WorkerParameters) :
  ...
  companion object {
    ...
    fun deleteUpdate(context: Context) {
      ...
      // 删除数据库中的更新记录
      scope.launch { hUpdateDao.delete() }
    }
    ...
    suspend fun enqueue(context: Context, latest: Latest) {
      ...
      val req =
        OneTimeWorkRequestBuilder<HUpdateWorker>()
          .addTag(TAG)
          .setConstraints(constraints)
          .setInputData(data)
          .build()
      workManager.beginUniqueWork(TAG, ExistingWorkPolicy.REPLACE, req).enqueue()
    }
      ...
      scope.launch {
        ...
        val req =
          OneTimeWorkRequestBuilder<HUpdateWorker>()
            ...
            .setInputData(data)
            .build()
        workManager.beginUniqueWork(TAG, ExistingWorkPolicy.REPLACE, req).enqueue().await()
      }
    }

    fun stop() {
      scope.launch {
        runSuspendCatching { workManager.cancelUniqueWork(TAG).await() }
          .onFailure { t -> // 上述方法可能无法取消任务
            ...
            val deleteRequest =
              OneTimeWorkRequestBuilder<HanimeDownloadWorker>()
                ...
                .build()
            workManager
              .beginUniqueWork(TAG, ExistingWorkPolicy.REPLACE, deleteRequest)
              .enqueue()
              .await()
          }
      }
    ...
    suspend fun collectOutput(context: Context) =
      WorkManager.getInstance(context).getWorkInfosByTagFlow(TAG).collect { workInfos ->
        // 只有一個！
  ...
  override suspend fun doWork(): Result {
    with(HUpdater) {
      ...
      try {
        ...
        file.injectUpdate(downloadLink, startOffset) { progress -> updateNotification(progress) }
        val outputData =
          workDataOf(
        ...
        result =
          if (e is CancellationException) {
            ...
            e.printStackTrace()
            mainScope.launch { showShortToast(e.localizedMessage) }
            file.delete()
        ...
        // 这也是一个历史遗留问题。
        scope.launch { hUpdateDao.updateState(state) }
      }
      return result
    }
  }
