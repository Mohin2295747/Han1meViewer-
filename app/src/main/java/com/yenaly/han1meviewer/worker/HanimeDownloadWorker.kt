...
class HanimeDownloadWorker(private val context: Context, workerParams: WorkerParameters) :
  ...
  private suspend fun download(): Result {
    return withContext(Dispatchers.IO) {
      val file =
        HFileManager.getDownloadVideoFile(videoCode, hanimeName, quality, suffix = videoType)
          ...
          ?: kotlin.run {
            ...
            DatabaseRepo.HanimeDownload.find(videoCode, quality)
              ?: return@withContext kotlin.run {
                ...
                mainScope.launch {
                  showShortToast(context.getString(R.string.download_task_failed_s, hanimeName))
                }
                Result.failure()
              }
          }
