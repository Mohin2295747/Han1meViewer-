...
object Config {
  ...
  object Version {
     ...
     * @return 版本号和版本名
     *
     *   Example: ci: 0.1.2-ci+21032500 release: 0.1.2-release+21032500 debug (fixed): debug+1
     */
    fun Project.createVersion(major: Int, minor: Int, patch: Int): Pair<Int, String> {
    ...
    val Project.source: String
      get() =
        System.getenv("HA1_VERSION_SOURCE")
          ?: kotlin.run {
  ...
  val Project.lastCommitSha: String
    get() =
      providers
        .exec { commandLine = "git rev-parse --short=7 HEAD".split(' ') }
        .standardOutput
        .asText
        .get()
        .trim()
