...
class SpannedTextGenerator private constructor() {
  ...
  class KotlinBuilder(private val flag: Int = Spanned.SPAN_EXCLUSIVE_EXCLUSIVE) {
    ...
    fun addText(
      ...
      if (isQuote) {
        ssb.setSpan(YenalyQuoteSpan(quoteColor, quoteStripeWidth, quoteGapWidth), start, end, flag)
      }
      if (isNewLine) {
    ...
    fun addImage(
      ...
      drawable?.let {
        ...
        ssb.setSpan(
          YenalyImageSpan(it, verticalAlignment, fontWidthMultiple, marginLeft, marginRight),
          start,
          end,
          flag,
        )
