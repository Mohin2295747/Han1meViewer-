...
fun View.resize(scaleX: Float, scaleY: Float) = updateLayoutParams {
  width = (width * scaleX).toInt()
  height = (height * scaleY).toInt()
}

/**
 * This function is used to remove a view from its parent ViewGroup. It checks if the parent of the
 * view is a ViewGroup and if so, removes the view from it.
 */
...
/**
 * This is a generic function that finds and returns the parent of a view that is of a specific
 * type. It traverses up the view hierarchy until it finds a parent of the specified type. If no
 * parent of the specified type is found, it throws an error.
 *
