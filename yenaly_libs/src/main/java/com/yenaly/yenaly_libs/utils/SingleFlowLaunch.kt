...
class SingleFlowLaunch {
  ...
  /**
   * Single [CoroutineScope.launch] only for ViewModelScope, avoid triggering request multiple times
   * when activity recreates and you call `viewModelScope.launch` function in activity or fragment.
   *
   * If you use [SharedFlow][kotlinx.coroutines.flow.SharedFlow] with [singleLaunch], you should set
   * the param `replay` to 1 or higher to cache the latest data. Otherwise, you might not get the
   * data.
   */
  fun singleLaunch(
